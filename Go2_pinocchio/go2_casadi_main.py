...
def generate_reference(
    ...
    for i in range(N):
        ...
        z_i = (cubic_bezier_interpolation(ca.DM(0), ca.DM(swing_height), 2*phase)
               if phase <= 0.5
               else cubic_bezier_interpolation(ca.DM(swing_height), ca.DM(0), 2*phase-1))

        # -------- world-frame foot positions at this phase --------------
        foot_w_dm = ca.reshape(                # 12×1 DM
            cubic_bezier_interpolation(p_foot_0, p_foot_1, phase), 12, 1
        )

        # -------- COM + yaw at this phase --------------------------------
        x_i_dm     = x_t[i]        # DM scalar
        y_i_dm     = y_t[i]        # DM scalar
        theta_i_dm = theta[i]      # DM scalar

        # -------- single call into the compiled CasADi function ----------
        q_i_dm, foot_body_flat_dm = reference_step(
            phase,            # scalar
            foot_w_dm,        # 12×1
            x_i_dm,           # scalar
            y_i_dm,           # scalar
            theta_i_dm,       # scalar
            z_i,              # scalar
            q_init_dm         # 12×1 initial guess
        )

        # -------- store results ------------------------------------------
        q_ref_cas[i, :]         = ca.reshape(q_i_dm,           1, 12)
        foot_ref_flat_cas[i, :] = ca.reshape(foot_body_flat_dm, 1, 12)
